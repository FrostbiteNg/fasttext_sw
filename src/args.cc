  ...
  for (int ai = 2; ai < args.size(); ai += 2) {
    ...
    try {
      if (args[ai] == "-h") {
        ...
        if (ws < 0) {
          std::cerr << "Parameter error: -ws must >= 0 !" << std::endl;
          printHelp();
          exit(EXIT_FAILURE);
        }
      } else if (args[ai] == "-sws") {
        sws = std::stoi(args.at(ai + 1));
        if (sws < 0) {
          std::cerr << "Parameter error: -sws must >= 0 !" << std::endl;
          printHelp();
          exit(EXIT_FAILURE);
        }
...
void Args::printTrainingHelp() {
  std::cerr
    ...
    << "  -ws                 size of the context window, invalid in supervised [" << ws << "]\n"
    << "  -sws                size of the skip context window, default off [" << sws << "]\n"
