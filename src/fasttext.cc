...
void FastText::supervised_sw(
  ...
  if (labels.size() == 0 || line.size() == 0) return;
  std::uniform_int_distribution<> uniform_label(0, labels.size() - 1);
  int32_t i = uniform_label(model.rng);    

  std::vector<int32_t> bow;
  std::uniform_int_distribution<> uniform(1, args_->sws);
  for (int32_t w = 0; w < line.size(); w+=args_->sws) {
  ...
  while (tokenCount_ < args_->epoch * ntokens) {
    ...
    if (args_->model == model_name::sup) {
      ...
      if (args_->sws == 0) {
        ...
        supervised_sw(model, lr, line, labels);
      }
